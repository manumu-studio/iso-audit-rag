...
# and upsert into the `controls` table. Idempotent — safe to re-run after
# schema or chunker changes. Run with: `uv run python scripts/ingest.py`.
import asyncio
import json
import time
...
def _build_embedding_input(chunk: ControlChunk) -> str:
    """Combine ID + title + description into the text we hand to the embedder."""
    return f"{chunk.id} {chunk.title}\n{chunk.description}"


def _format_vector(values: list[float]) -> str:
    """Format a float list into pgvector's text representation: '[v1,v2,...]'."""
